...
```{r}
choice2 <- sums2 %>% select(c(ArtDate, россии, URL))
...
choice2 %>%
  group_by(ArtDate) %>% 
  summarise(Frequency = sum(россии)) %>%
  ggplot(aes(x = ArtDate, y = Frequency)) +
  geom_point() +
  labs(
    title = "Term Frequency Per Day Over Time",
    subtitle = "Term: 'россии' (Russia), TASS Search Term: 'египет' (Egypt)",
    x = "Date",
    y = "Frequency"
...
ggsave("tass_images/russia2.png", width = 6, dpi = 500)
```
